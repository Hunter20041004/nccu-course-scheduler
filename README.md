...
5. 到「AI 推薦」輸入背景、目標與偏好，產生三個不衝堂的推薦方案。
6. 排好後使用「匯出手機桌布」，輸出雨後日光 × 輕夢核風格課表圖。
...
src/course-data.mjs         內建候選課程與政大課程 metadata
...
開啟終端顯示的本機網址即可使用。AI 功能需要使用者在介面中貼上自己的 Gemini API Key；一般排課、手動新增、衝堂檢查與桌布匯出不需要 key。
